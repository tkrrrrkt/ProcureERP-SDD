...
import type { TaxRateDto, TaxRateSortBy, SortOrder } from "../types/bff-contracts"
...
interface TaxRateListProps {
  onSelectTaxRate: (taxRate: TaxRateDto) => void
  onCreateTaxRate: () => void
}

export function TaxRateList({ onSelectTaxRate, onCreateTaxRate }: TaxRateListProps) {
  ...
  const [sortBy, setSortBy] = useState<TaxRateSortBy>((searchParams.get("sortBy") as TaxRateSortBy) || "taxRateCode")
  ...
  const loadTaxRates = useCallback(async () => {
    ...
    setError(null)
    try {
      const response = await bffClient.listTaxRates({
        page,
        pageSize,
  ...
  const handleSort = (column: TaxRateSortBy) => {
    if (sortBy === column) {
  ...
  const renderSortIcon = (column: TaxRateSortBy) => {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold">税率マスタ</h1>
        ...
        <div className="overflow-x-auto">
          <Table>
            <TableHeader>
              <TableRow>
                <TableHead
                  onClick={() => handleSort("taxRateCode")}
                  className="cursor-pointer hover:bg-muted"
                >
                  税率コード {renderSortIcon("taxRateCode")}
                </TableHead>
                <TableHead
                  onClick={() => handleSort("ratePercent")}
                  className="cursor-pointer hover:bg-muted text-right"
                >
                  税率（%） {renderSortIcon("ratePercent")}
                </TableHead>
                <TableHead
                  onClick={() => handleSort("validFrom")}
                  className="cursor-pointer hover:bg-muted"
                >
                  適用開始日 {renderSortIcon("validFrom")}
                </TableHead>
            ...
            <TableBody>
              {loading ? (
                ...
                taxRates.map((taxRate) => (
                  <TableRow
                    key={taxRate.id}
                    onClick={() => onSelectTaxRate(taxRate)}
                    className="cursor-pointer hover:bg-muted/50"
                  >
                    <TableCell className="font-mono">{taxRate.taxRateCode}</TableCell>
                    <TableCell className="text-right font-mono">{taxRate.ratePercent}</TableCell>
                    <TableCell>{formatDate(taxRate.validFrom)}</TableCell>
                    <TableCell>{formatDate(taxRate.validTo)}</TableCell>
                    <TableCell>
