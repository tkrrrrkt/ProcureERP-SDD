...
import { Search, Plus, ArrowUp, ArrowDown, FileX2, ChevronRight } from "lucide-react"
import type { ItemAttributeDto, ItemAttributeSortBy, SortOrder } from "../types/bff-contracts"
...
interface ItemAttributeListProps {
  onSelectAttribute: (attribute: ItemAttributeDto) => void
  onCreateAttribute: () => void
}

export function ItemAttributeList({ onSelectAttribute, onCreateAttribute }: ItemAttributeListProps) {
  const router = useRouter()
  const searchParams = useSearchParams()

  const [attributes, setAttributes] = useState<ItemAttributeDto[]>([])
  ...
  const [sortBy, setSortBy] = useState<ItemAttributeSortBy>((searchParams.get("sortBy") as ItemAttributeSortBy) || "sortOrder")
  ...
  const loadAttributes = useCallback(async () => {
    ...
    setError(null)
    try {
      const response = await bffClient.listItemAttributes({
        page,
        pageSize,
  ...
  const handleSort = (column: ItemAttributeSortBy) => {
    if (sortBy === column) {
  ...
  const renderSortIcon = (column: ItemAttributeSortBy) => {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold">品目仕様属性マスタ</h1>
            ...
            <TableBody>
              {loading ? (
                ...
                attributes.map((attribute) => (
                  ...
                  >
                    <TableCell className="font-mono text-sm">{attribute.attributeCode}</TableCell>
                    <TableCell className="font-medium">{attribute.attributeName}</TableCell>
                    <TableCell className="text-center">{attribute.sortOrder}</TableCell>
                    ...
                    <TableCell>
                      <Badge variant={attribute.isActive ? "default" : "outline"}>
