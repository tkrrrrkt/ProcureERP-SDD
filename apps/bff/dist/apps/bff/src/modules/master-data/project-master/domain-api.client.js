...
let DomainApiHttpClient = class DomainApiHttpClient {
    ...
    async list(tenantId, userId, request) {
        ...
        if (params.projectCode)
            queryParams.append('projectCode', String(params.projectCode));
        if (params.projectName)
            queryParams.append('projectName', String(params.projectName));
        if (params.projectShortName)
            queryParams.append('projectShortName', String(params.projectShortName));
        if (params.departmentCode)
            queryParams.append('departmentCode', String(params.departmentCode));
        if (params.responsibleEmployeeCode)
            queryParams.append('responsibleEmployeeCode', String(params.responsibleEmployeeCode));
        if (params.includeInactive)
            queryParams.append('includeInactive', String(params.includeInactive));
        const response = await fetch(`${this.baseUrl}/api/master-data/project-master?${queryParams.toString()}`, {
            method: 'GET',
            headers: {
    ...
    async findById(tenantId, userId, id) {
        const response = await fetch(`${this.baseUrl}/api/master-data/project-master/${id}`, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json',
                'x-tenant-id': tenantId,
    ...
    async create(tenantId, userId, request) {
        const response = await fetch(`${this.baseUrl}/api/master-data/project-master`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'x-tenant-id': tenantId,
    ...
    async deactivate(tenantId, userId, id, ifMatchVersion) {
        const response = await fetch(`${this.baseUrl}/api/master-data/project-master/${id}/deactivate`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'x-tenant-id': tenantId,
    ...
    async reactivate(tenantId, userId, id, ifMatchVersion) {
        const response = await fetch(`${this.baseUrl}/api/master-data/project-master/${id}/reactivate`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'x-tenant-id': tenantId,
